function [TG, TGD, taxis]  = mstraj2(segments, qdmax, tsegment, q, dt, Tacc, varargin)
% MSTRAJ Multi-segment multi-axis trajectory
%
%
% [TG, TGD, taxis]  = mstraj2(segments, qdmax, tsegment, q, dt, Tacc, varargin)
% Julian Leland (originally modified by Veevee Cai), MIT Media Lab,
% 2016-06-08
%
% TRAJ = MSTRAJ2(P, QDMAX, TSEG, Q0, DT, TACC, OPTIONS) is a trajectory
...
% http://www.petercorke.com
    ...
    for seg=1:ns
        ...
        [qb, qbd] = jtraj(q, q_prev+tacc2*qd, 0:dt:taccx, qd_prev, qd);
        tg = [tg; qb(2:end,:)];
        tgd = [tgd; qbd(2:end,:)];
    ...
    % add the final blend
    [qb, qbd] = jtraj(q, q_next, 0:dt:tacc2, qd_prev, qdf);
    tg = [tg; qb(2:end,:)];
    tgd = [tgd; qbd(2:end,:)];
